...
So..projectiles...
We want to fire at the creeps as they pass. The range we have chosen for a tower is 5 (meaning 5 tiles).
Let's get the firing animation first. All the work for this will be in our `tower_manager.go`
...
Then in UpdatePlacedTowers, right under where we determine the weapon angle, add this (effectively at the bottom).
...
So, once a creep come into range, the firing annimation will kick off. We will be using the end of that animation to trigger when we fire again.  
Note we have put a delay between shots to make things more reasonable.

Now that the weapon rotates to face the enemy, and fires off an animation to shoot, we need to add the projectile in the correct place, rotated at the correct angle and have it move toward the player.
Both our projectile sprites are animated, so we need to handle that too. We won't worry about impacts yet.

First thing, let us spawn the projectile. In the case of the bolt, it should be rotated to face the enemy where it was fired. Let's create our Projectile and Projectile Manager. Create a new file named `projectile.go`
...
Let's add a couple constants for environment

```go
const projectileSpeed = 12.0
const maxProjectileRange = 5.0
```

We need a factory for our ProjectileManager
...
Next we need to Spawn a Projectile, which simply adds it to the slice.
...
Then of course, we need to draw the projectiles. Note that the majority of the code here is just applying the proper rotation angle.
...
Next we move on to our Tower Manager and add this to it.

```go
projectileManager    *ProjectileManager
```

That makes our factory look like this
...
Next we put a provider for the Draw function.
...
Then we create a method in our tower manager for it to spawn a projectile.
...
Lastly we go to our GameScene and add this to the end of our Draw function

```go
g.towerManager.DrawProjectiles(screen, params, g.level)
```

Now we have the projectiles spawn and they are facing the right direction. We need to animate them, and move them next.

First let's edit Draw in our ProjectileManager
...
At the end of the flight, we will need to play the Impact Animation. Let's get that wired. It's just loading the animation up.
...
Then we want to add an Update function to move our Projectile.
...
It's pretty straightforward, and I'd added a TODO where we can hook into collision checking.
We will call that Update function from the TowerManager's update. Go to `tower_manager.go` and find UpdatePlacedTowers. At the very bottom of that function, add this.

```go
tm.projectileManager.Update(deltaTime, activeCreeps)
```

Run the game. Your projectiles now fire, travel their range of 5 tiles, and explode in a nice animated cloud. We need to check for collisions and kill creeps now. FINALLY!
The Update function in Creep needs some changes. Open up the file and replace the function call and the first line (only the first line, which is duplicated here).
...
You won't be able to build creep_manager now because of the function signature change. Find that line in the Update function and replace it with this

```go
creep.Update(deltaTime, level, onEscape, cm.onCreepKilled)
```

Only the last param is different, so it should be easy to find.
Next we wander into GameScene and the NewGameScene factory. We will need to set that callback here.
I'm just putting the whole function here for ease.
...
Now we go to ProjectileManager. Look in the Update function for the TODO. This is where the collision check should go, so paste this in there.
...
And let's create the checkCollisionWithCreeps function.
...
And now, run the game and it works! You now have a very basic tower defense. Take this and do a few things with it.

- Add More Creep Types
- Add More Tower Types
- Add a score (waves? creeps killed?)
- Make a leaderboard
- Add particles
- Fix the waypoints and add smoothing

These are just a few examples of things that could be done to turn this into something really good.

If you have any questions, feel free to ping me on Discord. The code can be found [here](https://github.com/RAshkettle/TowerDefenseTutorial). If you find an issue with the code, please create an issue in github so I can fix it for any future readers.
